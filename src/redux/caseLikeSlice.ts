...
>('caseLikes/fetchStatus', async (caseId, { rejectWithValue }) => {
  try {
    ...
    if (token) {
      ...
      const resp = await axios.get<{ liked: boolean }>(`${CASE_API_URL}/${caseId}`, config);
...
export const toggleCaseLike = createAsyncThunk(
  ...
  async ({ caseId }: { caseId: number }) => {
    ...
    if (token) {
      const config = { headers: { Authorization: `Bearer ${token}` } };
      const res = await axios.post(`${CASE_API_URL}/${caseId}/like`, {}, config);
...
const caseLikeSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchCaseLikeCount.fulfilled, (state, action: PayloadAction<{ caseId: number; count: number }>) => {
      ...
      .addCase(toggleCaseLike.fulfilled, (state, action: PayloadAction<{ caseId: number; liked: boolean }>) => {
        const { caseId, liked } = action.payload;
        state.loading[caseId] = false;
        state.liked[caseId] = liked;
        state.counts[caseId] = (state.counts[caseId] || 0) + (liked ? 1 : -1);
      })
      .addCase(toggleCaseLike.rejected, (state, action) => {
        const id = (action.meta.arg as any).caseId;
        state.loading[id] = false;
      })
      .addCase(fetchCaseLikeStatus.fulfilled, (state, action: PayloadAction<{ caseId: number; liked: boolean }>) => {
        const { caseId, liked } = action.payload;
        state.loading[caseId] = false;
        state.liked[caseId] = liked;
      });
